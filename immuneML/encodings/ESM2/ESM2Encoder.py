...
class ESM2Encoder(DatasetEncoder):
    def __init__(self, layers: list = None, name: str = None, pickle_file: str = None, embeddings_file: str = None, cdr3_sequence_file: str = None):
        ...
        #load file with cdr3 sequences into pandas dataframe from csv
        if cdr3_sequence_file is not None:
            self.cdr3_df = pd.read_csv(self.cdr3_sequence_file)
        else:
            self.cdr3_df = None
    ...
    def get_embeddings(self, data_loader, batches):
        ...
        with torch.no_grad():
            for batch_idx, (labels, strs, toks) in enumerate(tqdm(data_loader, desc="Processing batches")):
                ...
                if self.cdr3_sequence_file is not None:
                    # Filter self.cdr3_sequence_file to only include the sequences in the current batch
                    # This is necessary because the sequences in the file are not in the same order as the sequences in the batch
                    with open(self.cdr3_sequence_file, "r") as f:
