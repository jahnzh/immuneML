...
@dataclass
class DistanceMatrixHeatmap(EncodingReport):
    """
    This encoding report can be used to visualize distance matrix encodings (:ref:`DistanceEncoder` and :ref:`EditDistanceEncoder`)
    as a heatmap with clustering dendrograms.
    ...
    .. code-block:: yaml
        ...
        my_dme_report: DistanceMatrixHeatmap
    ...
    def build_object(cls, **kwargs):
        ParameterValidator.assert_keys_present(list(kwargs.keys()), ['file_format', 'name'],
                                               DesignMatrixExporter.__name__, DesignMatrixExporter.__name__)
        ParameterValidator.assert_in_valid_list(kwargs['file_format'], ['npy', 'csv', 'npy.zip', 'csv.zip', 'hdf5.zip'],
    ...
    def check_prerequisites(self):
        if self.dataset.encoded_data is None or self.dataset.encoded_data.examples is None:
            ...
            return False
        elif self.dataset.encoded_data.encoding not in [DistanceEncoder]:
            return False
        else:
            return True
